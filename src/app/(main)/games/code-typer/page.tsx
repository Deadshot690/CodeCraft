...
type Language = CodeTyperChallenge['language'] | 'all';
type Difficulty = CodeTyperChallenge['difficulty'] | 'all';

export default function CodeTyperLobbyPage() {
  ...
  const languages = useMemo(() => ['all', ...Array.from(new Set(codeTyperChallenges.map(q => q.language)))], []);
  const difficulties = useMemo(() => ['all', ...Array.from(new Set(codeTyperChallenges.map(q => q.difficulty)))], []);


  const filteredChallenges = useMemo(() => codeTyperChallenges.filter((challenge) => {
  ...
  const difficultyVariant = (difficulty: CodeTyperChallenge['difficulty']): "default" | "secondary" | "destructive" | "outline" => {
    switch (difficulty) {
      case "Beginner":
        return "default";
      case "Intermediate":
                  ...
                  <TableRow key={challenge.id}>
                    ...
                    <TableCell className="text-right">
                      <Button asChild size="sm">
                        <Link href={`/games/code-typer/${challenge.id}`}>
