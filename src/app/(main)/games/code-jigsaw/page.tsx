...
type Language = CodeJigsawChallenge['language'] | 'all';
type Difficulty = CodeJigsawChallenge['difficulty'] | 'all';
...
export default function CodeJigsawLobbyPage() {
  ...
  const languages = useMemo(() => ['all', ...Array.from(new Set(codeJigsawChallenges.map(q => q.language)))], []);
  const difficulties = useMemo(() => ['all', ...Array.from(new Set(codeJigsawChallenges.map(q => q.difficulty)))], []);


  const filteredChallenges = useMemo(() => codeJigsawChallenges.filter((challenge) => {
