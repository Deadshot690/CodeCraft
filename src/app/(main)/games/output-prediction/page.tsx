...
type Language = OutputPredictionChallenge['language'] | 'all';
type Difficulty = OutputPredictionChallenge['difficulty'] | 'all';

export default function OutputPredictionLobbyPage() {
  ...
  const languages = useMemo(() => ['all', ...Array.from(new Set(outputPredictionChallenges.map(q => q.language)))], []);
  const difficulties = useMemo(() => ['all', ...Array.from(new Set(outputPredictionChallenges.map(q => q.difficulty)))], []);


  const filteredChallenges = useMemo(() => outputPredictionChallenges.filter((challenge) => {
  ...
  const difficultyVariant = (difficulty: OutputPredictionChallenge['difficulty']): "default" | "secondary" | "destructive" | "outline" => {
    switch (difficulty) {
      case "Beginner":
        return "default";
      case "Intermediate":
    ...
    <div className="flex flex-col">
       <header className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b bg-background/80 px-4 backdrop-blur-sm md:px-6">
        <h1 className="font-headline text-xl font-bold tracking-tight md:text-2xl">
          Output Prediction: Choose a Challenge
        </h1>
      </header>
                  ...
                  <TableRow key={challenge.id}>
                    ...
                    <TableCell className="text-right">
                      <Button asChild size="sm">
                        <Link href={`/games/output-prediction/${challenge.id}`}>
